...
run_kernel.py → Process (state machine: DAG schedule / failure / retry / triage)
              ...
              → agent_transport (worker prompt → opencode/claude subprocess → AgentEvent)
...
- Backend selection: `run_kernel.py --backend opencode|claude`; per-agent backend in `agents_config.json`.
