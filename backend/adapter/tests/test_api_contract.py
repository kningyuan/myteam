...
OVERVIEW_KEYS = {"project_id", "title", "status", "mode", "workflow", "workflow_label", "launch_error",
...
def test_summary_totals_shape(tmp_path):
    from common.observability.observability import projects_summary
    store = Store(tmp_path / "state.db")
...
def test_real_app_mro_envelope_vs_default_404(tmp_path, monkeypatch):
    ...
    seed.upsert_project(pid, title="probe", status="in_progress")
    seed.close()
...
def test_resume_marks_run_status(tmp_path, monkeypatch):
    ...
    from common.runtime import run_kernel
    store = Store(tmp_path / "state.db")
