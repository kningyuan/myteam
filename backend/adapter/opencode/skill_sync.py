"""OpenCode 工作区 Skill 同步 — 将 myteam 挂载的 Skill 注册到 .opencode/skills/。"""
...
OPENCODE_SKILLS_DIR_NAME = "skills"
MANIFEST_NAME = ".myteam-skills.json"


def _skills_root(workspace: Path) -> Path:
    return workspace / OPENCODE_DIR_NAME / OPENCODE_SKILLS_DIR_NAME


def _manifest_path(workspace: Path) -> Path:
    return workspace / OPENCODE_DIR_NAME / MANIFEST_NAME


def sync_workspace_skills(workspace: str | Path, skill_ids: list[str]) -> dict:
    """把 skill_ids 同步到 workspace/.opencode/skills/<id>/（目录软链，非仅 SKILL.md）。"""
