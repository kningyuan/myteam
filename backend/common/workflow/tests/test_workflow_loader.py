#!/usr/bin/env python3
"""workflow_loader 与 PGD profile DAG 校验。"""
from __future__ import annotations

import pytest

from common.process.plan_gate import topological_order
from common.workflow.workflow_loader import (
    allocate_workflow_id,
    delete_workflow,
    list_workflows,
    load_workflow,
    read_workflow_raw,
    resolve_workflow_display_name,
    roster_from_tasks,
    write_workflow_raw,
)


def test_list_workflows_includes_profiles():
    ids = list_workflows()
    assert "方案完善" in ids
    assert "区块链产品规划-完善" in ids


def test_load_plan_improve_workflow_generic():
    profile = load_workflow("方案完善")
    assert profile.id == "方案完善"
    assert profile.name == "方案完善"
    assert len(profile.loops) == 1
    assert profile.loops[0].id == "plan_improve_round"
    assert "区块链" not in profile.description
    tasks = profile.instantiate_tasks(goal="完善某系统建设方案 Word 文档")
    assert tasks[0]["description"].startswith("【项目目标】完善某系统建设方案")


def test_allocate_workflow_id_skips_existing():
    existing = set(list_workflows())
    n = 1
    while f"workflow-{n}" in existing:
        n += 1
    assert allocate_workflow_id() == f"workflow-{n}"


def test_resolve_workflow_display_name():
    assert resolve_workflow_display_name("方案完善") == "方案完善"
    assert resolve_workflow_display_name("missing-wf-xyz") == "missing-wf-xyz"


@pytest.mark.parametrize("wid", ["方案完善"])
def test_load_workflow_passes_plan_gate(wid: str):
    profile = load_workflow(wid)
    assert profile.id == wid
    assert "main" in profile.roster
    tasks = profile.instantiate_tasks(goal="测试目标注入")
    assert tasks[0]["description"].startswith("【项目目标】测试目标注入")
    order = topological_order(tasks)
    assert order[0] in {t["id"] for t in tasks}
    assert len(order) == len(tasks)


def test_roster_from_tasks_derives_agents():
    tasks = [
        {"id": "t1", "agent": "research", "task_type": "research", "dependencies": []},
        {"id": "t2", "agent": "product", "task_type": "research", "dependencies": ["t1"]},
    ]
    assert roster_from_tasks(tasks) == ["main", "research", "product"]


def test_write_read_delete_workflow_raw(monkeypatch, tmp_path):
    monkeypatch.setattr("common.workflow.workflow_loader.workflows_dir", lambda: tmp_path)
    data = {
        "id": "ui-test-flow",
        "name": "UI 测试流程",
        "version": "1.0",
        "description": "测试",
        "options": {"parallel_enabled": False},
        "tasks": [
            {
                "id": "t1",
                "name": "调研",
                "agent": "research",
                "task_type": "research",
                "dependencies": [],
                "description": "【对象】x",
            },
        ],
    }
    wid = write_workflow_raw(data)
    assert wid == "ui-test-flow"
    raw = read_workflow_raw(wid)
    assert raw["description"] == "测试"
    assert "roster" not in raw
    profile = load_workflow(wid)
    assert profile.roster == ["main", "research"]
    assert profile.id == wid
    delete_workflow(wid)
    assert wid not in list_workflows()


def test_allocate_workflow_id_on_create(monkeypatch, tmp_path):
    monkeypatch.setattr("common.workflow.workflow_loader.workflows_dir", lambda: tmp_path)
    wid = write_workflow_raw({
        "name": "新建流程",
        "version": "1.0",
        "description": "自动 id",
        "tasks": [{
            "id": "t1",
            "name": "步骤",
            "agent": "research",
            "task_type": "research",
            "dependencies": [],
            "description": "测试",
        }],
    })
    assert wid == "workflow-1"
    assert read_workflow_raw(wid)["name"] == "新建流程"
    delete_workflow(wid)
