...
from common.workflow.workflow_collaboration import (
...
def test_project_fallback_without_store(monkeypatch):
    monkeypatch.setattr(
        "common.workflow.workflow_collaboration.collaboration_for_project",
        lambda pid: collaboration_from_options({}),
    )
    monkeypatch.setattr("common.skill.skill_settings.is_auto_group_enabled", lambda: True)
    monkeypatch.setattr("common.skill.skill_settings.is_project_group_enabled", lambda: True)
...
def test_collaboration_for_project_reads_meta(monkeypatch):
    ...
    monkeypatch.setattr("common.store.store.Store", FakeStore)
