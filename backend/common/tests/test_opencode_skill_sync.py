...
from adapters.opencode.skill_sync import (  # noqa: E402
    MANIFEST_NAME,
...
def skill_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("adapters.opencode.skill_sync.SKILLS_DIR", skills_dir)
...
def test_sync_links_desired_skills(skill_env):
    ...
    assert (ws / ".opencode" / "skills" / "alpha" / "SKILL.md").is_file()
    assert (ws / ".opencode" / "skills" / "beta" / "SKILL.md").is_file()
    manifest = json.loads((ws / ".opencode" / MANIFEST_NAME).read_text(encoding="utf-8"))
    assert manifest["skill_ids"] == ["alpha", "beta"]


def test_sync_removes_unmounted_skills(skill_env):
    ws, skills_dir = skill_env
    stale = ws / ".opencode" / "skills" / "stale"
    ...
    assert not (ws / ".opencode" / "skills" / "stale").exists()
    assert (ws / ".opencode" / "skills" / "alpha" / "SKILL.md").is_file()
...
def test_adapter_sync_agent_skills(skill_env, monkeypatch):
    ...
    assert (ws / ".opencode" / "skills" / "alpha" / "SKILL.md").read_text(encoding="utf-8").startswith("# alpha")
