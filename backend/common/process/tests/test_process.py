...
import common.process.process as process_mod  # noqa: E402
from common.agent.agent_port import AgentPort, WatchdogConfig  # noqa: E402
from common.process.process import (Process, ProcessConfig, PlanCheckResult,
...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("common.agent.agent_id_policy.BUSINESS_CONFIG_DIR", bc)
...
def test_recurring_completed_extracts_skill_draft(env, tmp_path, monkeypatch):
    ...
    store, wcfg = env
    monkeypatch.setattr("common.skill.skill_extract.MYTEAM_ROOT", tmp_path)
    monkeypatch.setattr("common.skill.skill_extract.SKILLS_DIR", tmp_path / "business/skills")
...
def test_normalize_and_splice_units():
    ...
    from common.process.plan_splice import normalize_subtasks, splice_subtasks
