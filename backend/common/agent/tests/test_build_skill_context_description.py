...
from common.agent.agent_skills import build_skill_context  # noqa: E402


def test_build_skill_context_includes_name_and_description():
    with patch(
        "common.agent.agent_skills.get_agent_info",
        return_value={"skills": ["demo-skill"]},
    ), patch(
        "common.agent.agent_skills.get_skill_library_entry",
        return_value={
            "id": "demo-skill",
            "name": "演示 Skill",
        ...
        "common.agent.agent_skills.skill_file_path",
        return_value=Path("/tmp/business/skills/demo-skill/SKILL.md"),
    ), patch(
        "common.agent.agent_skills.workspace_dir",
