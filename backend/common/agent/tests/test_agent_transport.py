...
from common.agent.agent_transport import (  # noqa: E402
    AdapterTransport,
    build_worker_prompt,
...
def test_transport_injects_rules_file(env, tmp_path, monkeypatch):
    ...
    import common.agent.agent_transport as _at_mod
...
def test_transport_uses_per_agent_backend(env, monkeypatch):
    ...
    monkeypatch.setattr("common.agent.agent_transport._default_adapter", fake_default)
...
def test_transport_two_agents_different_backends(env, monkeypatch):
    ...
    monkeypatch.setattr("common.agent.agent_transport._default_adapter", lambda b: TaggedAdapter(b))
...
def test_config_reloads_from_disk_when_not_injected(monkeypatch):
    ...
    monkeypatch.setattr("common.agent.agent_transport._load_agents_config", load)
...
def test_empty_model_falls_back_to_settings_default(monkeypatch):
    monkeypatch.setattr(
        "common.agent.agent_transport._load_agents_config",
        lambda: {"research": {"backend": "opencode", "model": ""}},
    )
    monkeypatch.setattr(
        "common.agent.agent_transport.resolve_agent_model",
...
def test_parallel_execute_workspace_isolated(tmp_path, monkeypatch):
    ...
    from common.agent.agent_transport import parallel_execute_workspace
