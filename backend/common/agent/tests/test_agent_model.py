...
from common.agent.agent_model import (  # noqa: E402
...
def test_resolve_uses_explicit_model(tmp_path, monkeypatch):
    ...
    cfg_dir.mkdir()
    monkeypatch.setattr("common.agent.agent_model.BUSINESS_CONFIG_DIR", cfg_dir)
...
def test_resolve_falls_back_to_settings_default(tmp_path, monkeypatch):
    ...
    cfg_dir.mkdir()
    monkeypatch.setattr("common.agent.agent_model.BUSINESS_CONFIG_DIR", cfg_dir)
    monkeypatch.setattr("common.agent.agent_model.system_default_backend", lambda: "opencode")
    write_agents_config({"research": {"backend": "opencode", "model": ""}})
    monkeypatch.setattr(
        "config_store.system_config.system_config.get_default_model",
...
def test_resolve_backend_from_settings_when_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("common.agent.agent_model.system_default_backend", lambda: "claude")
...
def test_ensure_entries_does_not_write_model(tmp_path, monkeypatch):
    ...
    cfg_dir.mkdir()
    monkeypatch.setattr("common.agent.agent_model.BUSINESS_CONFIG_DIR", cfg_dir)
    monkeypatch.setattr("common.agent.agent_model.WORKSPACES_DIR", tmp_path / "workspaces")
    monkeypatch.setattr("common.agent.agent_model.WORKSPACE_PREFIX", "workspace-")
    monkeypatch.setattr("common.agent.agent_model.system_default_backend", lambda: "opencode")
    write_agents_config({})
