...
def build_worker_prompt(req, resp_path: Path, deliv_dir: Path,
    ...
    lines += [
        "",
        "【提交结果（最后一步 — 必须执行，否则任务不算完成）】",
        "交付物写完后，**必须**按以下两步提交（缺一不可）：",
        f"  步骤 1：用 write 工具把下面的 JSON 写入 /tmp/result.json",
        f"  步骤 2：用 bash 工具运行：{sys.executable} {submit_script} --out {resp_path} --file /tmp/result.json",
        "  （注意 --out 路径必须与上面完全一致）",
        "",
        "结果 JSON 模板（替换尖括号内容）：",
        "{",
        ...
        "}",
        "",
        "⚠️ 写完 /tmp/result.json 后**立即**运行 submit_result 命令，不要在聊天里返回 JSON。",
        "只有 submit_result 成功写入 .response 文件，任务才算完成。否则会被看门狗判定为超时。",
    ]
    return "\n".join(lines)
