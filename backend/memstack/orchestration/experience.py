...
def promote_ledger_to_memory(
    ...
    content = raw
    try:
        from execution_harness.config import ledger_distill_enabled
        from execution_harness.post.distill import distill_ledger_body

        if ledger_distill_enabled():
            distilled = distill_ledger_body(raw, task_type=task_type, task_id=task_id)
            if distilled.strip():
                content = distilled
    except Exception:
        pass
    ...
    return kb.write(
        ...
        content,
        task_id=task_id,
        tags=[task_type, "ledger", "distilled"],
    )
