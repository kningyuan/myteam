#!/usr/bin/env python3
"""REG-O8：结构化审计日志（CHECK_ONLY）。

验证 request/response snapshot 含 hash、agent_id、outcome，且篡改可检测。

用法:
    MYTEAM_ROOT=$PWD PYTHONPATH=$PWD/backend \
        python3 scripts/regression/reg_o8_audit_log.py
"""
from __future__ import annotations

import os
import subprocess
import sys
from pathlib import Path

_REPO = Path(__file__).resolve().parents[2]


def main() -> int:
    print("=== REG-O8 结构化审计日志（CHECK_ONLY）===")
    cmd = [
        str(_REPO / "venv" / "bin" / "python3"),
        "-m",
        "pytest",
        "backend/common/tests/test_audit_log.py",
        "-q",
    ]
    env = {**os.environ, "PYTHONPATH": str(_REPO / "backend")}
    proc = subprocess.run(cmd, cwd=str(_REPO), env=env, capture_output=True, text=True)
    if proc.stdout:
        print(proc.stdout.rstrip())
    if proc.stderr:
        print(proc.stderr.rstrip(), file=sys.stderr)
    ok = proc.returncode == 0
    print(f"REG-O8: {'PASS' if ok else 'FAIL'}")

    sys.path.insert(0, str(_REPO / "scripts" / "regression"))
    from regression_archive import append_run_record  # noqa: E402

    rec = append_run_record(
        reg_id="REG-O8",
        project_id="reg-o8-audit-log",
        pass_=ok,
        kpis={"exit_code": proc.returncode},
        meta={"mode": "check_only", "test": "backend/common/tests/test_audit_log.py"},
    )
    print(f"  I-06 archived: run_id={rec['run_id']}")
    return 0 if ok else 1


if __name__ == "__main__":
    sys.exit(main())
