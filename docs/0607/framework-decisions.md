...
- **抽象层 → 新模块（全部位于 `skill/team/common/`，均可单测，旧引擎未动可回退）**：
  ...
  - AgentPort 真实传输 → `agent_transport.py`（Phase 8 step 1）
...
- **Phase 8（go-live 收尾，进行中）**：
  1. ✅ **AgentPort 真实 Transport**：`common/agent_transport.py`（`AdapterTransport` 驱动 `backend/adapters/opencode`，把 `AgentEvent` 流转发到 `ctx.emit` 作心跳；提示词约束 agent 用 `submit_result` 写回；适配器可注入便于测试/换 claude）。AgentPort 增 `cancel_event` 暴露 + `step_finish` token 计量（D17）。已加全栈集成测试 `test_integration.py`（Process→AgentPort→AdapterTransport(fake opencode)→Gate→Store→Observability 跑通一个 DAG）。
  ...
  5. ✅ **现网切换 + 清理（完成）**：agent 面向的调度文档（`task-executor`/`continuous-executor`/`project-init` 的 `SKILL.md`）已改为指向 `run_kernel`（one_shot / `--mode recurring`），旧引擎脚本仅作回退保留。**依据**：grep 证明旧引擎仅由 SKILL.md 调起、无 Hub/服务以库形式 import，故切入口安全可回退。**已做 5(b)-i 解耦通知路径**：`project_data.py` / `group_notify.py` 移除 telegram 子进程调用（仅走项目群+UI）。**已做 5(b)-ii 删旧引擎 + telegram**：删除 `skill/team/{task-executor,continuous-executor,notify-telegram}/` 三个自洽目录及其测试（含 `paths` 的孤儿 `executor_script`/`notify_telegram_script`）；保留 `quality_gate.py`（新内核 `gate.py` 复用其证据校验）与 `graph_gate.py`（`notify_agent`/`task_queue`/`dispatch`/`init` 仍用）。删后 Hub/`notify_agent`/`project_data`/`gate`/`run_kernel` 均 import 正常、全仓测试 102 例绿。**已做 5(b)-iii 清孤儿模块**：删除仅旧引擎使用、现零引用的 `cross_review`/`validator`/`deliverable_merger`/`resume_engine`/`response_finder`（+ `test_cross_review`）。**已做 5(b)-iv 删 gbrain 死回退**：`quality_gate.read_standards` 去掉 gbrain CLI 分支（删 `_read_standards_from_gbrain`/`_run_gbrain_get`/`_parse_standards_page`/`_simple_yaml_parse` + `GBRAIN_TIMEOUT`），标准只读本地注册表 `templates.yaml`；`gate.py` 复用的证据校验工具（`verify_published_url`/`_URL_RE`/`_extract_field`/`EVIDENCE_VERIFY_OFF`）原样保留；`test_quality_gate` 的 5 个 gbrain 用例合并为 2 个本地用例。**已做 5(c) config/path + ARCHITECTURE**：确认 config/path 已结构化收敛（`store.system_config` 为配置真相、`base/system_config` 仅兼容垫片 re-export；server 用 `hub/paths.py`、worker 用 `common/paths.py`，均以 `MYTEAM_ROOT` 为根、`config/` 为配置目录，无定义漂移）；`docs/ARCHITECTURE.md` 新增 §11 编排内核（run_kernel→Process→AgentPort→Gate/Registry→Store→Observability）与 §12 config·path 出处，并在 §7 标注双引擎已迁移到 Process 内核。**已做 5(b)-v 删 JSON 抢救开关**：经全仓核查确认 `_write_response_file`（F1 抢救）零调用点（原调用者 `executor` 已删）、`contracts_enabled()` 仅此一处消费、现网通知走 `hub.services.notify_service`——均为死代码，遂删 `notify_agent._write_response_file` 整段 F1 抢救 + `contracts.contracts_enabled()` + `INTERACTION_CONTRACTS` 开关（及孤儿 `import os`）。**Phase 8（go-live 切换 + 清理）至此全部完成**。**切换后真机回归（5(b)-v 删抢救之后）**：真实 opencode 经 `run_kernel` goal 驱动跑通 `team_config→task_plan→research(researcher)→seo-plan(seo)→strategy(analyst)`，`status=completed`、3 任务 needs_review（D18 不阻塞）、总 86k token；产出 3 份 deliverable.md + 合法契约 `.response`（`interaction_id/kind/status/quality/result/notes`，`result.outcome` 齐备）——证明契约唯一路径（submit_result+Gate，无 F1 抢救）端到端成立。**补强（task_plan agent∈team 硬校验）**：真机回归暴露 Main 会指派名册外 agent（analyst）漏网执行；遂在 `Process._task_plan` 加硬校验——每个任务的 `agent` 必须 ∈ team_config 选出的名册，越界则带 feedback 重试（`max_plan_retries`，默认 2），耗尽仍越界则显式 `RuntimeError`（不让漏网 agent 进调度）；`agent_transport` 同步把 `retry_feedback` 注入决策类提示词（原仅 execute 注入）。新增 3 例（`test_process` 2 + `test_agent_transport` 1），全仓 **94 例绿**。
- **测试**：全仓 **91 例绿**（删旧双引擎 ~39 例 + cross_review 8 例 + gbrain 回退 5→2 用例后；新栈含全栈集成 1 + Hub 可观测 6 + 运行时入口 3）。
- **工作约定（改动纪律）**：
...
- **Hub / 交互层（`backend/`）**——运行时，已较成熟：
  ...
  - CLI 实例：`backend/adapters/opencode/`（`adapter.py` 子进程跑 `opencode run --dir ... -f rules`、`parser.py`）；`backend/adapters/claude/`（驱动 `claude -p --output-format stream-json`）。
