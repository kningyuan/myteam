...
class Executor:
    ...
    def try_resume(self, project_name: str) -> bool:
        ...
        projects_dir = BASE / "tasks" / "projects"
    ...
    def state_evaluate_task(self, task_id: str, agent_id: str) -> Optional[dict]:
        ...
        retry_count = 0
        while time.time() < deadline:
            found_resp = find_response_file(self.project_id, agent_id, task_id, expected_phase="evaluate")
            if found_resp:
                try:
    ...
    def state_execute_task(self, task_id: str, agent_id: str, parent_task: Optional[str] = None, sub_task_index: Optional[str] = None) -> bool:
        ...
        # Step 4: 清理可能残留的旧 response 文件（如评估阶段遗留的）
        ...
        retry_count = 0
        while time.time() < deadline:
            found_resp = find_response_file(self.project_id, agent_id, task_id, expected_phase="execute")
            if found_resp:
                try:
                    response = json.loads(found_resp.read_text(encoding="utf-8"))
    ...
    def _notify_task_event(self, event_type: str, agent_id: str, task_id: str, *extra_args, **kwargs):
        """直接调用 notify-telegram 发送任务事件（开始/完成/失败/拆分）。"""
        notify_script = str(SKILLS / "notify-telegram" / "scripts" / "notify.py")
        if not Path(notify_script).exists():
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_SCRIPT_NOT_FOUND]", "warning")
            return
        # 处理 None agent_id（如 review 失败时未指定通知对象）
        safe_agent = agent_id or ""
        safe_extra = tuple(str(a) for a in extra_args if a is not None)
        # 从 kwargs 中提取 feedback 并作为额外参数传递（用于任务重试的详细原因）
        feedback = kwargs.get("feedback", "")
        if feedback and "feedback" not in [a for a in extra_args]:
            safe_extra = safe_extra + (feedback,)
        rc, stdout, stderr = _run_script(
            notify_script, self.project_id, event_type, safe_agent, task_id,
            *safe_extra, timeout=NOTIFY_EVENT_TIMEOUT
        )
        if rc == 0:
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_OK] event={event_type}, task={task_id}")
        else:
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_FAIL] event={event_type}, task={task_id}, stderr={stderr[:200]}", "warning")
            # 兜底：写入日志文件
            log_path = project_dir(self.project_id) / "group-notifications.log"
            log_path.parent.mkdir(parents=True, exist_ok=True)
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"[{datetime.now().isoformat()}] [{event_type}] task={task_id}, agent={agent_id}\n")

    def _notify_subtask_event(self, event_type: str, agent_id: str, parent_task_id: str, subtask_id: str, sub_task_index: int = 0):
        """直接调用 notify-telegram 发送子任务事件（创建/开始/完成/失败）。"""
        notify_script = str(SKILLS / "notify-telegram" / "scripts" / "notify.py")
        if not Path(notify_script).exists():
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_SCRIPT_NOT_FOUND] path={notify_script}", "warning")
            return
        rc, stdout, stderr = _run_script(
            notify_script, self.project_id, event_type, agent_id, parent_task_id, subtask_id, timeout=NOTIFY_EVENT_TIMEOUT
        )
        if rc == 0:
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_OK] event={event_type}, subtask={subtask_id}")
        else:
            self.log(f"[EXECUTOR_NOTIFY_TELEGRAM_FAIL] event={event_type}, subtask={subtask_id}, stderr={stderr[:200]}", "warning")
            log_path = project_dir(self.project_id) / "group-notifications.log"
            log_path.parent.mkdir(parents=True, exist_ok=True)
            with open(log_path, "a", encoding="utf-8") as f:
