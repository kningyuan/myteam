...
"""
Agent 间消息通知 Skill
通过 openclaw agent -m 命令实现 Agent 间通信
...
def get_project_data(project_id):
    ...
    project_file = Path.home() / f".openclaw/tasks/projects/{corrected_id}/task_data.json"
...
def _extract_agent_reply(stdout: str) -> str:
    """从 openclaw agent --json 输出中提取 Agent 的回复内容。

    openclaw agent --json 的输出结构为:
    {
      "runId": "...",
      "status": "ok",
      "summary": "completed",
      "result": {
        "payloads": [{"content": "Agent 回复文本", ...}],
        "meta": {...}
      }
    }

    如果 payloads 有内容则提取，否则返回空字符串。
    """
    import re
    try:
        # 跳过 stderr 的 UNDICI 警告（以 (node: 开头）
        clean = re.sub(r'^\(node:\d+\).*\n', '', stdout, flags=re.MULTILINE)
        clean = clean.strip()
        start = clean.find('{')
        if start < 0:
            return ""
...
def send_agent_message(agent_id, message, timeout=AGENT_MSG_TIMEOUT, thinking='minimal', project_id=None, deliver=False):
    """
    通过 openclaw agent -m 发送消息给指定 Agent

    Args:
        deliver: 是否将 Agent 的回复投递到 Telegram 频道
    """
    logger = get_skill_logger(project_id) if project_id else None
    openclaw_agent = AGENT_ID_MAP.get(agent_id, agent_id)
    ...
    # 当需要获取 Agent 回复内容时加 --json；纯通知不加（更快）
    if not deliver:
        cmd.append('--json')
    if deliver:
        cmd.append('--deliver')
    ...
    if logger:
        logger.info(f"[AGENT_MSG_SEND] agent={agent_id}, project={project_id}, thinking={thinking}, deliver={deliver}, timeout={timeout}, msg_preview={msg_preview}", extra={'skill_name': 'AGENT_NOTIFY'})
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            timeout=timeout,
            cwd=str(Path.home() / ".openclaw" / f"workspace-{openclaw_agent}")
        )

        if result.returncode == 0:
            stdout = result.stdout.strip()
            # openclaw agent --json 返回的是 Gateway 运行状态JSON，Agent 实际回复不在 stdout 中
            # 尝试从 JSON 中提取 agent 回复，如果没有则返回空字符串（由调用方处理）
            extracted = _extract_agent_reply(stdout)
            resp_preview = extracted[:300].replace('\n', '\\n')[:300] if extracted else stdout[:300].replace('\n', '\\n')[:300]
            if logger:
                logger.info(f"[AGENT_MSG_SUCCESS] agent={agent_id}, project={project_id}, resp_preview={resp_preview}", extra={'skill_name': 'AGENT_NOTIFY'})
            return True, extracted or stdout
        else:
            err_preview = result.stderr[:500].replace('\n', '\\n')
            if logger:
                logger.error(f"[AGENT_MSG_FAIL] agent={agent_id}, project={project_id}, error={err_preview}", extra={'skill_name': 'AGENT_NOTIFY'})
            return False, result.stderr
    except subprocess.TimeoutExpired:
        if logger:
            logger.error(f"[AGENT_MSG_TIMEOUT] agent={agent_id}, project={project_id}, timeout={timeout}s", extra={'skill_name': 'AGENT_NOTIFY'})
        return False, "Timeout: Agent 调用超时"
    except Exception as e:
        if logger:
            logger.error(f"[AGENT_MSG_ERROR] agent={agent_id}, project={project_id}, error={str(e)}", extra={'skill_name': 'AGENT_NOTIFY'})
        return False, str(e)
...
请读取 ~/.openclaw/workspace-{agent_id}/.trigger/{corrected_id}_{task_id}.trigger 获取任务详情并开始执行。

【重要】开始执行前，请先调用以下命令更新任务状态为 in_progress（将触发群通报）：
exec: ~/.openclaw/skills/team-ok/project-data/scripts/project_data.py update-task {project_id} {task_id} in_progress

执行完毕后，请在 .response/ 目录返回 JSON 响应。"""

    trigger_file = Path.home() / f".openclaw/workspace-{agent_id}/.trigger/{corrected_id}_{task_id}.trigger"
    ...
    success, output = send_agent_message(agent_id, message, project_id=project_id, deliver=False)
...
def notify_task_complete(project_id, task_id, agent_id, require_valid_graph=False):
    ...
    deliverables_dir = Path.home() / f".openclaw/tasks/projects/{corrected_id}/deliverables"
    deliverables = list(deliverables_dir.glob(f"{task_id}_*.md")) if deliverables_dir.exists() else []
...
任务已完成，交付物已保存到 deliverables/ 目录。"""
    ...
    success, output = send_agent_message('main', message, project_id=project_id, deliver=True)
...
请读取 ~/.openclaw/workspace-{agent_id}/.trigger/{corrected_id}_{task_id}.trigger 获取任务详情。
...
不要直接调用任何 skill 脚本。"""
    ...
    success, output = send_agent_message(agent_id, message, timeout=timeout_value, project_id=project_id, deliver=False)

    if success:
        _write_response_file(corrected_id, agent_id, task_id, output, expected_phase="evaluate")
        ...
        # ACK 模式下超时说明消息已成功发送给 Agent
        if ack_timeout and "Timeout" in output:
            logger.info(f"[NOTIFY_EVALUATE_ACK] project={project_id}, task={task_id}, agent={agent_id}, timeout={ack_timeout}s",
                        extra={'skill_name': 'AGENT_NOTIFY'})
            return True, f"已通知 {AGENT_NAME_MAP.get(agent_id, agent_id)} 评估任务（ACK 模式）"
        logger.error(f"[NOTIFY_EVALUATE_END] project={project_id}, task={task_id}, agent={agent_id}, status=fail, error={output}",
                     extra={'skill_name': 'AGENT_NOTIFY'})
        return False, f"通知失败：{output}"
...
def notify_task_execute(project_id, task_id, agent_id, parent_task=None, sub_task_index=None, ack_timeout=None):
    ...
    deliverable_path = str(deliverables_dir / deliverable_filename)
    ...
    # 从 .trigger 文件读取模板内容
    agent_ws = Path.home() / f".openclaw/workspace-{agent_id}"
    trigger_path = agent_ws / ".trigger" / f"{corrected_id}_{task_id}.trigger"
    template_guide = ""
...
请读取 ~/.openclaw/workspace-{agent_id}/.trigger/{corrected_id}_{task_id}.trigger 获取完整任务详情。

【重要】执行完成后，请将以下 JSON 写入文件，不要通过聊天回复。聊天中的内容会被忽略。
文件：~/.openclaw/workspace-{agent_id}/.response/{corrected_id}_{task_id}.response
{{
  "phase": "execute",
  "task_id": "{task_id}",
  "status": "completed",
...
不要直接调用任何 skill 脚本。"""
    ...
    if success:
        _write_response_file(corrected_id, agent_id, task_id, output, expected_phase="execute")
        logger.info(f"[NOTIFY_EXECUTE_END] project={project_id}, task={task_id}, agent={agent_id}, status=success",
                    extra={'skill_name': 'AGENT_NOTIFY'})
        return True, f"已通知 {AGENT_NAME_MAP.get(agent_id, agent_id)} 执行任务"
    else:
        # ACK 模式下超时说明消息已成功发送给 Agent（Agent 已开始工作但未完成）
        if ack_timeout and "Timeout" in output:
            logger.info(f"[NOTIFY_EXECUTE_ACK] project={project_id}, task={task_id}, agent={agent_id}, timeout={ack_timeout}s",
                        extra={'skill_name': 'AGENT_NOTIFY'})
            return True, f"已通知 {AGENT_NAME_MAP.get(agent_id, agent_id)} 执行任务（ACK 模式）"
        logger.error(f"[NOTIFY_EXECUTE_END] project={project_id}, task={task_id}, agent={agent_id}, status=fail, error={output}",
                     extra={'skill_name': 'AGENT_NOTIFY'})
        return False, f"通知失败：{output}"
...
def _write_response_file(project_id: str, agent_id: str, task_id: str, raw_output: str, expected_phase: str = ""):
    ...
    import re
    agent_ws = Path.home() / f".openclaw/workspace-{agent_id}"
    resp_file = agent_ws / ".response" / f"{project_id}_{task_id}.response"
...
def notify_team_config(project_id, agent_id):
    """通知 Agent 配置团队，写回 .response 文件"""
    logger = get_skill_logger(project_id)
    agent_ws = Path.home() / f".openclaw/workspace-{agent_id}"
    req_file = agent_ws / ".response" / f"{project_id}_team_config.request"
    resp_file = agent_ws / ".response" / f"{project_id}_team_config.response"
    ...
    success, output = send_agent_message(agent_id, message, project_id=project_id, deliver=False, thinking=thinking_mode)
    if not success:
        return False, output

    # 写 .response 文件（从 output 中提取 JSON）
    resp_content = output.strip()
    # 尝试从 markdown 代码块中提取 JSON（支持对象 {} 和数组 []）
    import re
    ...
    try:
        ...
        # Agent 未返回有效 JSON → 使用默认团队配置
        # 解析项目描述中的关键词来推断角色
        project_msg = req.get('message', '')
        default_agents = ['product', 'developer', 'tester']
        if any(kw in project_msg for kw in ['设计', 'ui', 'UX', '界面', '视觉']):
            default_agents.append('designer')
        if any(kw in project_msg for kw in ['运营', '内容', '小红书', 'social', '社媒']):
            default_agents.extend(['content', 'social'])
        if any(kw in project_msg for kw in ['调研', '研究', '分析', '竞品']):
            default_agents.append('researcher')
        if any(kw in project_msg for kw in ['运维', '部署', 'ci/cd', 'devops']):
            default_agents.append('ops')
        if any(kw in project_msg for kw in ['文档', 'doc', 'manual']):
            default_agents.append('docs')
        parsed = {'agents': list(dict.fromkeys(default_agents))}
        logger.info(f"[TEAM_CONFIG_FALLBACK] Agent 无回复，使用默认团队: {parsed['agents']}")

    resp_file.parent.mkdir(parents=True, exist_ok=True)
    with open(resp_file, "w", encoding="utf-8") as f:
        json.dump(parsed, f, ensure_ascii=False, indent=2)
    logger.info(f"[TEAM_CONFIG_SUCCESS] response written: {resp_file}")
    return True, "团队配置已完成"
...
def notify_task_plan(project_id, agent_id):
    ...
    logger = get_skill_logger(project_id)
    agent_ws = Path.home() / f".openclaw/workspace-{agent_id}"
    req_file = agent_ws / ".response" / f"{project_id}_task_plan.request"
    resp_file = agent_ws / ".response" / f"{project_id}_task_plan.response"
    ...
    try:
        parsed = json.loads(resp_content)
        # Agent 有时返回 [{...}]（数组包对象），自动提取
        if isinstance(parsed, list) and len(parsed) == 1 and isinstance(parsed[0], dict):
            parsed = parsed[0]
    except json.JSONDecodeError:
        logger.warning(f"[TASK_PLAN] Agent 返回非 JSON，原文保存: {resp_content[:200]}")
        parsed = {"raw_response": resp_content}
        if not resp_content.strip():
            # Agent 未返回有效 JSON → 使用默认单任务规划
            project_msg = req.get('message', '')
            parsed = {
                "tasks": [
                    {
                        ...
                        "reviewer": "tester",
                        "description": project_msg[:200],
                        "dependencies": [],
                    }
                ]
            }
            logger.info(f"[TASK_PLAN_FALLBACK] Agent 无回复，使用默认单任务规划")

    resp_file.parent.mkdir(parents=True, exist_ok=True)
    with open(resp_file, "w", encoding="utf-8") as f:
        json.dump(parsed, f, ensure_ascii=False, indent=2)
    logger.info(f"[TASK_PLAN_SUCCESS] response written: {resp_file}")
    return True, "任务规划已完成"
...
def notify_json_response_request(
    ...
    logger = get_skill_logger(project_id)
    agent_ws = Path.home() / f".openclaw/workspace-{agent_id}"
    req_file = agent_ws / ".response" / f"{project_id}_{stem}.request"
    resp_file = agent_ws / ".response" / f"{project_id}_{stem}.response"
    ...
    success, output = send_agent_message(
        agent_id, message, project_id=project_id, deliver=False, thinking=thinking_mode
    )
    if not success:
        return False, output
