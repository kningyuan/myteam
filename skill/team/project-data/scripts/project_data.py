...
TEAM_SKILL_DIR = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(TEAM_SKILL_DIR))
...
def get_data_file(project_id):
    ...
    data_file = task_data_path(corrected_id)
...
def get_lock_file(project_id):
    """获取项目锁文件路径（带自动修正）"""
    corrected_id = normalize_project_id(project_id)
    return project_dir(corrected_id) / ".task.lock"
...
def get_current_task_from_trigger(agent_id=None):
    ...
    # 查找 trigger 目录
    tdir = trigger_dir(agent_id)
    
    if not tdir.exists():
        print(f"Error: trigger 目录不存在: {tdir}", file=sys.stderr)
        return None, None
    
    trigger_files = list(tdir.glob("*.trigger"))
...
def send_notification(project_id, event_type, *args, sync=False):
    """发送项目进度通报到 myteam 项目群（Telegram 默认关闭）。"""
    try:
        from common.notify_format import format_event_message
        from bridge.myteam_notify import post_project_group_message

        agent_id = args[0] if len(args) > 0 else ""
        task_id = args[1] if len(args) > 1 else (args[0] if event_type.startswith("project") else "")
        extra = " ".join(str(a) for a in args[2:]) if len(args) > 2 else ""

        if event_type.startswith("subtask"):
            agent_id = args[0] if len(args) > 0 else ""
            task_id = args[2] if len(args) > 2 else ""
            extra = args[3] if len(args) > 3 else ""

        data = read_task_data(project_id)
        project_name = (data.get("project") or {}).get("name", project_id) if data else project_id
        task_name = ""
        deliverables = []
        if data and task_id:
            for t in data.get("tasks") or []:
                ...
                for st in t.get("subtasks") or []:
                    if st.get("id") == task_id:
                        ...
                        break
            deliv_dir = deliverables_dir(project_id)
            if deliv_dir.is_dir():
                deliverables = sorted(f.name for f in deliv_dir.glob(f"{task_id}*.md"))

        subtask_name = task_name if event_type.startswith("subtask") else ""
        message = format_event_message(
            ...
            deliverables=deliverables if event_type.endswith("complete") else None,
            extra=extra,
        )

        post_project_group_message(project_id, event_type, agent_id, task_id, message=message, extra=extra)

        try:
            skill_logger = get_skill_logger(project_id)
            skill_logger.info(
                f"[NOTIFY_SENT] event={event_type}, mode={'sync' if sync else 'async'}, args={args}",
            ...
            for task in data['tasks']:
                if task['id'] == task_id:
                    ...
                    if status == 'in_progress' and not task.get('started_at'):
                        ...
                        # 自动释放队列锁（让其他任务可以执行）
                        corrected_id = normalize_project_id(project_id)
                        queue_file = project_dir(corrected_id) / ".task_queue"
...
from common.paths import TEAM_SKILL_DIR, registry_file
...
def register_skill(skill_id, name, category, path, description):
    ...
    # 验证路径存在
    skill_path = TEAM_SKILL_DIR / path
