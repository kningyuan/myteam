...
所有 Skill 的日志都记录到：~/team/projects/{project_id}/skill-logs/skills.log
...
from common.paths import LOGS_DIR, project_dir
...
def get_skill_logger(project_id: str) -> logging.Logger:
    ...
    with _lock:
        ...
        # 构建项目目录和日志文件路径
        proj_dir = project_dir(corrected_id)
        log_file = proj_dir / "skill-logs" / "skills.log"
        
        # 【关键】验证项目是否已初始化（检查 task_data.json）
        task_data_file = proj_dir / "task_data.json"
...
def log_skill_step_failure(
    ...
    `~/team/logs/uninitialized_projects.log`，仍优于完全无记录。
    """
...
def _get_temp_logger(project_id: str, corrected_id: str) -> logging.Logger:
    ...
    error_log = LOGS_DIR / "uninitialized_projects.log"
