...
集中管理 `~/team/projects/{project_id}/task_data.json` 的读写、
...
def deliverables_dir(project_id: str) -> Path:
    return _deliverables_dir(project_id)


def trigger_dir(agent_id: str) -> Path:
    return _trigger_dir(agent_id)


def response_dir(agent_id: str) -> Path:
    return _response_dir(agent_id)
